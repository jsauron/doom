...
/*   Updated: 2019/09/21 23:16:55 by jsauron          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		hud_life(t_game *game, t_pos pos, int old_time)
{
	if ((is_inmap(&pos) && (is_inwall(&pos, game, NULL) == 2))
		&& game->player.life > 0)
	{
...
void		hud_keys_life(t_game *game, t_pos pos, int old_time)
{
	...
	else if ((is_inmap(&pos) && (is_inwall(&pos, game, NULL) == 8)))
	{
		game->key = 5;
		game->map[(int)pos.y / BLOC_SIZE][(int)pos.x / BLOC_SIZE] = 0;
	}
	else if ((is_inmap(&pos) && (is_inwall(&pos, game, NULL) == 9)))
