...
Uint32 colorkey(SDL_Surface *src)
{
	Uint32 key;
...
int _zoomSurfaceRGBA(SDL_Surface * src, SDL_Surface * dst, int flipx, int flipy, int smooth)
{
	...
	t_colorRGBA *c00, *c01, *c10, *c11;
	t_colorRGBA *sp, *csp, *dp;
	...
	sp = (t_colorRGBA *) src->pixels;
	dp = (t_colorRGBA *) dst->pixels;
...
void _transformSurfaceRGBA(SDL_Surface * src, SDL_Surface * dst, int cx, int cy, int isin, int icos, int flipx, int flipy, int smooth)
{
	...
	t_colorRGBA c00, c01, c10, c11, cswap;
	t_colorRGBA *pc, *sp;
	int gap;
		...
		for (y = 0; y < dst->h; y++)
		{
			...
			for (x = 0; x < dst->w; x++)
			{
				...
				if ((dx >= 0) && (dy >= 0) && (dx < src->w) && (dy < src->h)) {
					sp = (t_colorRGBA *)((Uint8 *) src->pixels + src->pitch * dy);
					sp += dx;
					*pc = *sp;
				}
				sdx += icos;
...
void transformSurfaceY(SDL_Surface * src, SDL_Surface * dst, int cx, int cy, int isin, int icos, int flipx, int flipy)
{
	...
	ax = (cx << 16) - (icos * cx);
	ay = (cy << 16) - (isin * cx);
	pc = (t_colory*) dst->pixels;
	gap = dst->pitch - dst->w;

	memset(pc, (int)(colorkey(src) & 0xff), dst->pitch * dst->h);
